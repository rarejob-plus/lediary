...
export function renderEntry(root, id) {
  ...
  `;
    ...
    appendSection(root, '日記を膨らませる', renderExpansionSection(entry.expansionQuestions), false);
    root.appendChild(renderMockBanner());
}
function appendSection(parent, title, body, openByDefault) {
...
function renderShadowingSection(text) {
    const wrap = document.createElement('div');
    wrap.innerHTML = `
  ...
  `;
    wrap.querySelector('#gen').addEventListener('click', () => {
        ...
        wrap.querySelector('#gen').textContent = '再生 ▶︎';
    });
    return wrap;
}
function renderExpansionSection(questions) {
    const wrap = document.createElement('div');
